...
from binance_funcs import account_bal, get_size
        ...
        for pair in short_pairs:
            ...
            if not pd.isna(df.at[len(df)-1, 'signals']):
                ...
                if in_pos:
                    ...
                    if buy_sig:
                        stp = float(df.at[len(df)-1, 'signals'][5:24])
                        risk = (price - stp) / price
                        size = get_size(price, fixed_risk, balance, risk)
                        positions[pair] = 1                    
                        note = f"{now} buy {size} {pair} @ {price}, {stp}"
                        push = pb.push_note(now, note)
                        print(note)
