...
def market_benchmark(session) -> None:
    ...
    for x in session.pairs_data.keys():
        ...
        if last_stamp > (now - window):  # if there is data up to the last 4 hours
            ...
            if x == 'BTCUSDT':
                ...
                logger.info(f"btc benchmark scores: {btc_1d:.1%}, {btc_1w:.1%}, {btc_1m:.1%}")
            elif x == 'ETHUSDT':
                eth_1d = df.roc_1d.iloc[-1]
                eth_1w = df.roc_1w.iloc[-1]
                eth_1m = df.roc_1m.iloc[-1]
                logger.info(f"eth benchmark scores: {eth_1d:.1%}, {eth_1w:.1%}, {eth_1m:.1%}")
    ...
    logger.debug(f'1d median based on {len(all_1d)} data points')
    logger.debug(f'1w median based on {len(all_1w)} data points')
    logger.debug(f'1m median based on {len(all_1m)} data points')
    ...
    if session.live:
        logger.debug(f'pairs with recent data: {len(all_1d)} / {all_pairs}')
    ...
    for row in bal_data[::-1]:
        ...
        if row_dt > (now - timedelta(days=30)):  # and not bal_1m:
            try:
                ...
                logger.exception("strat_benchmark bal_1m produced attribute error")
                continue
        if row_dt > (now - timedelta(days=7)):  # and not bal_1w:
            try:
                bal_1w = row.get('balance')
            except AttributeError:
                logger.excpetion("strat_benchmark bal_1w produced attribute error")
                continue
        if row_dt > (now - timedelta(days=1)):  # and not bal_1d:
            try:
                bal_1d = row.get('balance')
            except AttributeError:
                logger.exception("strat_benchmark bal_1d produced attribute error")
                continue
...
def interpret_benchmark(session, agents: list) -> None:
    ...
    if d_ranking:
        ...
        for e, r in enumerate(d_ranking):
            logger.info(f'rank {e + 1}: {r[1]}% {r[0]}')
    if w_ranking:
