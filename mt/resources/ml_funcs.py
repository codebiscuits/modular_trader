...
def get_margin_pairs(method, num_pairs, history):
    ...
    pairs = [p for p in pairs if symbol_margin.get(p)
             and (market_info.at[p, 'length'] > history)
...
def add_features(df, tf):
    ...
    df = features.regime_osc_1(df, 12)
    df = features.regime_osc_1(df, 25)
    df = features.regime_osc_1(df, 50)
    df = features.regime_osc_1(df, 100)
    df = features.regime_osc_1(df, 200)
    ...
    df = features.smoothed_stoch(df, 20, True)
    df = features.smoothed_stoch(df, 50, True)
    df = features.smoothed_stoch(df, 100, True)
    df = features.smoothed_stoch(df, 200, True)
