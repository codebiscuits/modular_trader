...
def generate_dataset(pairs):
    ...
    for pair in pairs:
        ...
        print(f"{pair} data length: {len(df)}")
        # df = df.tail(data_len).reset_index(drop=True)
        res = backtest_oco(df, side, lookback)
        all_res.extend(res)
        gd_current = time.perf_counter()
        gd_elapsed = gd_current - gd_start
        print(f"Time elapsed since start of generate_dataset: {int(gd_elapsed // 60)}m {gd_elapsed % 60:.1f}s")
