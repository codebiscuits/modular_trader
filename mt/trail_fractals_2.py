...
targets - ie PnL > 0. So just remember that if the scores are really weird, then this may be a stupid thing to do."""
...
def save_models(side, tf, width, atr_spacing, feature_names, thresh, X_val, model, scaler):
    ...
    info_dict = {'features': feature_names, 'pnl_threshold': thresh, 'valid': len(X_val) > 30, 'validity': len(X_val)}
...
def trail_fractals_2(side, tf, width, atr_spacing, thresh):
    ...
    print(f"{X.shape = }, {y.shape = }, {X_val.shape = }, {y_val.shape}")
