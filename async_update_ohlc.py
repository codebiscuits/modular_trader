...
# TODO maybe work out a workflow to apply to each pair where it tries to load from disk, if it can then it updates, if
#  it can't then it downloads from scratch, then it cleans the data and finally saves to disk, and wrap all that in an
#  async function, and that can be the tasks in the list of coroutines. i might be able to borrow a lot from the
#  original update_ohlc module
...
async def main():
    ...
    for pair, klines in zip(pairs, results):
        ...
        df.to_parquet(f"test_ohlc/{pair}_5m.parquet")
