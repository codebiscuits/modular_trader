...
# TODO next job is to turn mlf.trail_fractal into trail_fractal_entries and backtest_trail_fractals (it would really
#  help if i could think of a name for the entry strategy).
...
def generate_dataset(pairs, side, timeframe, entry, exit, params, data_len):
    all_res = []
    for pair in pairs:
...
def final_rf_train_and_save(X_final, y_final, final_features, best_params, pairs, selection_method, strategy,
                            strat_params, data_len):
    ...
    if strategy == 'channel_run_200':
        strat = "channel_run"
        params = f"{strat_params['lookback']}"
    elif strategy == 'trail_fractals_5_2':
        strat = 'trail_fractals'
        params = f"{strat_params['width']}_{strat_params['spacing']}"
...
def train_primary(side, timeframe, strategy, num_pairs, selection_method, data_len, num_trials):
    loop_start = time.perf_counter()
    now_start = datetime.now().strftime('%Y/%m/%d %H:%M')

    if strategy == 'channel_run_200':
        entry = channel_run_entries
        exit = backtest_oco
        strat_params = {'lookback': 200}
        print(f"\n- Running Channel Run 1 {now_start}, {side}, {timeframe}, {strat_params['lookback']}, {num_pairs}, "
              f"{selection_method}")
    # elif strategy == 'trail_fractals_5_2':
    #     entry = trail_fractal_entries
    #     exit = backtest_trail_fractals
    #     strat_params = {'width': 5, 'spacing': 2}
    #     print(f"\n- Running Trail Fractals 1 {now_start}, {side}, {timeframe}, {strat_params['width']}, "
    #           f"{strat_params['spacing']}, {num_pairs}, {selection_method}")

    # generate dataset
    pairs = mlf.get_margin_pairs(selection_method, num_pairs)
    res_df = generate_dataset(pairs, side, timeframe, entry, exit, strat_params, data_len)
    ...
    # train final model
    final_rf_train_and_save(X_final, y_final, final_features, best_params, pairs, selection_method, strategy, strat_params, data_len)
...
for side, timeframe in product(sides, timeframes):
    train_primary(side, timeframe, 'channel_run_200', 150, '1w_volumes', 5000, 1000)
