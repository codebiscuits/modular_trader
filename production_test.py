...
for timeframe in timeframes:
    ...
    #         'weighted_50_bullish_doji': {'call': features.doji, 'params': (df, 0.5, 2, True)},
    #         'weighted_50_bearish_doji': {'call': features.doji, 'params': (df, 0.5, 2, True)},
    #         'weighted_100_bullish_doji': {'call': features.doji, 'params': (df, 1, 2, True)},
    #         'weighted_100_bearish_doji': {'call': features.doji, 'params': (df, 1, 2, True)},
    #         'weighted_150_bullish_doji': {'call': features.doji, 'params': (df, 1.5, 2, True)},
    #         'weighted_150_bearish_doji': {'call': features.doji, 'params': (df, 1.5, 2, True)},
    #         'un_weighted_bullish_doji': {'call': features.doji, 'params': (df, 0.5, 2, False)},
    #         'un_weighted_bearish_doji': {'call': features.doji, 'params': (df, 0.5, 2, False)},
    #         'recent_bullish_doji': {'call': features.doji, 'params': (df, 0.5, 2, False)},
    #         'recent_bearish_doji': {'call': features.doji, 'params': (df, 0.5, 2, False)},
    ...
    #         'fractal_trend_age_long': {'call': features.fractal_trend_age, 'params': (df, 'long')},
    #         'fractal_trend_age_short': {'call': features.fractal_trend_age, 'params': (df, 'short')},
    ...
    for pair in pairs:
        ...
        for f in feature_set:
            ...
            if f == 'r_pct':
                continue
            df = features.add_feature(df, f, timeframe)
        ...
        print(f"{pair} {timeframe} long conf: {long_confidence:.1%} short conf: {short_confidence:.1%}")

        combined_long = long_confidence - short_confidence
        combined_short = short_confidence - long_confidence

        if combined_long > 0.75:
            note = f"Buy {timeframe} {pair} @ {df.close.iloc[-1]} confidence: {combined_long:.1%}\n"
            print(note)
            notes += note
        if combined_short > 0.75:
            note = f"Sell {timeframe} {pair} @ {df.close.iloc[-1]} confidence: {combined_short:.1%}\n"
            print(note)
            notes += note
